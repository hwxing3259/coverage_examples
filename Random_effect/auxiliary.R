...
hpd<-function(CL,lev=0.95,test=NA,AA) {
...
#AA: vector of fectors
#S: list of partitions
#i: index of factor to change
GenerateCandidate<-function(AA,S,i) {
  ...
  if (gp==0) {
    ...
    #otherwise we write AA "a" into the chosen (existing) group
    Sp[[gp]]=c(Sp[[gp]],a)
  }
...
CooM<-function(CL,AA,com=NA,coc.file=NA,sam.file=NA) {
  ...
  if (is.na(com)) {
    Nsamp=dim(CL)[1] #J/SS;
    com=matrix(0,nl,nl,dimnames=list(AA,AA))
    for (i in 1:nl) {
      for (j in 1:nl) {
        for (m in 1:Nsamp) {
  ...
  #dispaly MDS projection of COOM - color/label by AA properties
  AA.dat=read.csv("AAP.csv",header=TRUE) #load some info about AA's in same order as AA
  xy=sammon(d,y=matrix(runif(2*nl),nl,2),magic=0.51,trace=FALSE)$points

  if (!is.na(sam.file)) {pdf(sam.file)} else {windows();}
  plot(xy,col=1+(AA.dat$MW>145),pch=4*(2+as.numeric(AA.dat$Side.chain.polarity)),ann=FALSE)#,
  #     xlim=c(-0.6,0.7),ylim=c(-0.4,0.6))
  text(xy,AA,pos=3,col=1+(AA.dat$MW>145),cex=0.6)
  if (!is.na(sam.file)) {dev.off()}

  #Principal components if you like
  #windows(); plot(cmd<-cmdscale(d),col=1+(AA.dat$MW>145),pch=4*(2+as.numeric(AA.dat$Side.chain.polarity)))
  #text(cmd,AA,pos=3,col=1+(AA.dat$MW>145),cex=0.6)
  #as.character(AA.dat[,1]) #full AA names
