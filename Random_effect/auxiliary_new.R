ooCORE<-function(PartSim,F,bn,sb,AA) {
...
comf=CooM(CLf,AA)
if (is.VE) {comv=CooM(CLv,AA)}

print(sprintf("%s",apply(hpd(CL=CLf,lev=0.5,AA=AAinclude)$set,1,paste,collapse=' ')))

###
#MAP cluster (max in run)
#mi=which.max(PL[,1]+PL[,2])
#prettyPart(mg2gm(CLf[mi,],AA))

#ML cluster (max in run)
#mj=which.max(PL[,1])
#prettyPart(mg2gm(CLf[mj,],AA))
...
makeSdata<-function(dataset,thin,synth=FALSE,base=NA,glmm=NA,prior=NA,St=NA,uniq.cov=FALSE,cut.cov=NA,keep.cov=NA) {
  ...
  #must remove "total" which is an aggregate over AA not an AA itself
  all=all[all$Name!="total",]

  b=all[is.element(all$Name,thin),]
  b$Name=factor(b$Name)
  n=dim(b)[1]

  #seems strange but Stage is uneven offsets 0,120,300 and Day is binary 0,7 anyway
  #and pressure has just 5 levels and isnt linear so might as well go categorical there
  b$Stage=as.factor(b$Stage)
  if (dataset=="PEF") {
    b$Day=as.factor(b$Day)  
  } else {
    b$Pressure=as.factor(b$Pressure) 
  }
  
  #create batch index marker to handle repeated measurements analysis showed that a batch 
  #is the set of 3 observations with the same Stage, Day, Process/Pressure and Meat value 
  #(and any Name value) these show clear correlation

  #b has a factor indicating batch
  if (dataset=="PEF") {
    X=model.matrix(Value~Stage+Process+Day+Meat,data=b)  
  } else {
    X=model.matrix(Value~Stage+Pressure+Meat,data=b)  
  }
  p=dim(X)[2]
  v=2^(0:(p-1))
  ...
  if (synth) {
    ...
    Bt=bd
    Bt$St=as.factor(gm2mg(St,Bt$Name)) #St defined in calling level
...
PriorPartSimS<-function(n,N,AA,alpha) {

  CL=matrix(NA,N,length(AA))
  
  for (k in 1:N) {
    g2=DP.sim(alpha,n=length(AA))
...
#thin=AA;synth=TRUE;St=Strue
#simulate from the given partition?
LkdYSim<-function(CL,n,N,AA,Y.dat,dataset,base,glmm,prior,uniq.cov=FALSE,cut.cov=NA,keep.cov=NA) {
  Y.sim=matrix(NA,n,N)
  d=rep(NA,N)
  for (k in 1:N) {
    Strue=mg2gm(CL[k,],AA)
    bd=makeSdata(dataset,thin=AA,synth=TRUE,base,glmm,prior,St=Strue,uniq.cov,cut.cov,keep.cov)
    Y.sim[,k]=bd$Y
...
#Start state
Sf=as.list(AA) #initial groups for FE or RE clustering
Kf=length(Sf)
