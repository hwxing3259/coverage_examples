...
logpr.joint <- function(ysynth, anchor, B){
  ...
  for (s in anchor){
    ...
    B$Y = ysynth
    b.m=lmer(formula = Y ~ DDDD * SSSS + (-1 + SSSS | Sf) + (-1 + SSSS | NNNN),
...
priorysimulate <- function(B, listpart){
  ZZlable = levels(B$NNNN)
  #listpart = mg2gm(part, ZZlable)
  B$Sf = as.factor(gm2mg(listpart, B$NNNN))
  b.m=lmer(formula = Y ~ DDDD * SSSS + (-1 + SSSS | Sf) + (-1 + SSSS | NNNN),
           data = B, REML = T)
...
priorfunc <- function(B){
  ...
  ysim = priorysimulate(B, mg2gm(part, levels(B$NNNN)))
...
runMCMCmodified<-function(B,EST,is.VE,F,J,SS,WTFI,alpha=1,VARY.ALPHA=FALSE,w.alpha=NA,APR=NA,SAVE_TO_FILE=FALSE,outfile=NA,DEBUG=FALSE,PART) {
  ...
  #Af: the group lable for each NNNN
...
partitionreg72 <- function(tryregression,B){
  ...
  for (i in 1:length(tryregression)){
    ...
    if (!is.na(ci[i])){
      ...
      Si[i,] = aggregate(Bsim$Y, list(Bsim$SSSS, Bsim$DDDD, Bsim$NNNN),mean)[,4]
    }
  }
...
quadrandseq <- function(obs,B){
  quadrant = list()
  for (i in c("DDDD", "SSSS", "NNNN")){
    factorvec = B[,i]
    quadrantseq = rep(NA, length(levels(factorvec)))
    l = 1
    for (j in levels(factorvec)){
      quadrantseq[l] = mean(obs[which(factorvec == j)])
      l = l+1
    }
